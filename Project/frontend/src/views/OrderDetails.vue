<template>
  <div>
    <button @click="goToOrderHistory" class="back-button">
    ...
    <div class="order-summary">
      ...
      <div v-if="orderDetails">
        ...
        <p>
          ...
          {{ formatDate(orderDetails.DateOrdered) }}
        </p>
        ...
        <p>
          ...
          {{ formatDate(orderDetails.DateScheduled) }}
        </p>
        <p>
          <strong>Date Delivered:</strong>
          {{ formatDate(orderDetails.DateDelivered) }}
        </p>
      </div>
      ...
      <div v-if="orderDetails.STATUS.Status === 'Unapproved'">
...
export default {
  ...
  methods: {
    async fetchOrderFeedback(OrderID) {
      console.log('Inside Fetched Feedback');
      try {
        const response = await fetch(
          `http://localhost:8080/customerData/Feedback/${OrderID}`,
          {
            method: 'GET',
    ...
    async fetchOrderDetails() {
      try {
        const response = await axios.get(
          `http://localhost:8080/customerData/Orders/${this.$route.params.id}`
        );
