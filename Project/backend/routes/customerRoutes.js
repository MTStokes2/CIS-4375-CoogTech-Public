...
router.post('/Orders/:id/products', async (req, res) => {
  ...
  try {
    ...
    const updatedTotal =
      ...
      parseFloat(product.ProductPrice) * parseInt(Quantity);
...
router.delete('/Orders/:id/products', async (req, res) => {
  ...
  try {
    ...
    const reduction =
      parseFloat(product.ProductPrice) * parseInt(productQuantity);
    ...
    res
      .status(201)
      .json({ message: 'Product removed from order successfully' });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Internal server error' });
  }
...
router.get('/Orders/:id/products', async (req, res) => {
  ...
  try {
    ...
    if (products.length > 0) {
      ...
      res
        .status(404)
        .json({ message: 'No products found for the given OrderID' });
    }
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Internal server error' });
...
router.post('/Orders', async (req, res) => {
  try {
    const parsedDateScheduled = moment(
      req.body.DateScheduled,
      'MM/DD/YYYY'
...
router.post('/customer/message', async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: 'Customer message saved and broadcasted successfully' });
  } catch (error) {
    // Handle errors
    console.error(error);
    res.status(500).json({ error: 'Internal server error' });
